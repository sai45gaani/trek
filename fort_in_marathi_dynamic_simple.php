...
function getDifficultyInMarathi($marathiGrade) {
    // If GradeMar is already in Marathi, return as is
    if (!empty($marathiGrade)) {
        return $marathiGrade;
    }
    // Fallback: convert English to Marathi
    return 'मध्यम';
}

// Slug generation - using English FortName for URL consistency
...
// Fetch FortNameMar, FortName, GradeMar, FortTypeMar from mi_tblfortinfo_unicode table
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        $slug = slugify($fortNameEng); // Use English name for slug
    ...
    <section class="py-12 bg-white dark:bg-gray-900">
        <div class="container mx-auto px-4">
            ...
            <div class="alphabet-filter">
                <?php 
                // Display only letters that have forts
            ...
            <?php if(isset($fortsData[$currentFilter])): ?>
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="fortsGrid">
                    <?php foreach($fortsData[$currentFilter] as $fort): ?>
                        <div class="fort-card bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-xl border border-gray-200 dark:border-gray-700 searchable-fort" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            fortCards.forEach(function(card) {
                ...
                // Search in both Marathi and English names
                if (fortName.includes(searchTerm) || fortNameEng.includes(searchTerm)) {
    ...
    function updateResultsCount(count) {
        ...
        console.log(`${count} किल्ले दिसत आहेत`);
    }
    ...
    function showSuggestions(matches, searchTerm) {
        suggestionsContainer.innerHTML = matches.map(fort => {
            ...
            return `
                <div class="suggestion-item px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer border-b border-gray-100 dark:border-gray-600 last:border-b-0 transition-colors" data-name="${fort.name}">
    ...
    console.log('महाराष्ट्रातील किल्ले (मराठी) पृष्ठ यशस्वीरित्या लोड झाले');
});
