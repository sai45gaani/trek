...
<body class="bg-gray-100 font-sans">
    ...
    <script>
// ===== GLOBAL FUNCTIONS FOR FORTS MANAGEMENT =====
let currentFortId = null;
let isEditMode = false;

// Load forts page via AJAX
...
function viewFort(fortId) {
    ...
    document.getElementById('modal-title').textContent = 'View Fort Details';
...
function editFort(fortId) {
    ...
    document.getElementById('modal-title').textContent = 'Edit Fort';
...
function displayFortDetails(data, editable) {
    ...
    if (editable) {
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <select id="fort-type" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all">
                            ...
                            <option value="Hill forts" ${data.FortType === 'Hill forts' ? 'selected' : ''}>Hill forts</option>
                            <option value="Sea forts" ${data.FortType === 'Sea forts' ? 'selected' : ''}>Sea forts</option>
                            <option value="Land forts" ${data.FortType === 'Land forts' ? 'selected' : ''}>Land forts</option>
                        </select>
                    </div>
        ...
        // VIEW MODE - Modern read-only layout with left-right structure
        html += `
            <!-- Basic Information Section -->
            <div class="border-b pb-4">
                <h3 class="text-lg font-semibold text-gray-800 mb-4 flex items-center">
...
function saveFort() {
    ...
    // Get editor content
    const intro = (typeof tinymce !== 'undefined' && tinymce.get('fort-intro')) ? tinymce.get('fort-intro').getContent() : document.getElementById('fort-intro')?.value || '';
    const history = (typeof tinymce !== 'undefined' && tinymce.get('fort-history')) ? tinymce.get('fort-history').getContent() : document.getElementById('fort-history')?.value || '';
...
function closeModal() {
    ...
    document.getElementById('fort-modal').classList.add('hidden');
    currentFortId = null;
    isEditMode = false;
}
